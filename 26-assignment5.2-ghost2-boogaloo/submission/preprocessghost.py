...
def analyze_one_flow(flow_packet_list):
    ...
    min_time_string = str(min_time)
    max_time_string = str(max_time)

    # with parts assembled, make the dictionary!
    analysis_dict = {
            "total_bytes_from_server" : total_bytes_from_server,
            "min_time" : min_time_string,
            "max_time" : max_time_string,
            }
...
def analyze_ghost(ghost_json_path="ghost2024.json",
    ...
    with open(ghost_json_path, "r") as ghost2024:
        ...
        page_flows_and_totals = group_flows_by_page(analysis_dict)

    return page_flows_and_totals

def group_flows_by_page(flow_dict):
    """
    function to  take in the flow dictionary containing:
        total bytes
        min time int
        max time int

    and group the flows into pages, returning a dictionary where:
        key : value
        index : {
    ...
    """
    end_time_to_flows = defaultdict(dict)
    for flow,flow_details_dict in flow_dict.items():
        end_time = flow_details_dict['max_time']
        total_flow_bytes = flow_details_dict['total_bytes_from_server']

        time_bucket = end_time_to_flows[end_time]

        curr_total_bytes = time_bucket.get('total bytes', 0)
        curr_total_bytes += total_flow_bytes

        curr_flow_count = time_bucket.get('total flows', 0)
        curr_flow_count += 1

        time_bucket['total bytes'] = curr_total_bytes
        time_bucket['total flows'] = curr_flow_count

        flows = time_bucket.get('flows', {})
        flows[flow] = total_flow_bytes
        time_bucket['flows'] = flows


    output_visit_order_page_list = []
    int_keys = []
    for key in end_time_to_flows.keys():
        int_keys.append(int(key))

    sorted_keys = sorted(int_keys)
    for key in sorted_keys:
        page_dict = end_time_to_flows[str(key)]
        page_dict['end time'] = key
        output_visit_order_page_list.append(page_dict)

    return output_visit_order_page_list
...
if __name__ == "__main__":
    ...
    with open('ghost2024_page_to_flows_analysis.json', 'w') as ghost_analysis_json_file:
